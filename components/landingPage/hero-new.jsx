...
// //           <div ref={containerRef} className="relative mt-4! 2xl:-mt-2 z-10">
// //             <nav className="relative z-50 flex items-center px-4 md:px-8 py-0">
// //               <div className="flex items-center gap-4 md:pl-16">
// //                 <div className="h-12 w-12 bg-[black] flex items-center justify-center">
// //                   <span className="font-bold text-xl text-white">UXB</span>
// //                 </div>
...
//     // Step 1: After 800ms, animate logo from center → top-left
//     const timer1 = setTimeout(() => {
//       if (!logoRef.current) return;
...
//     // Step 2: Fade out overlay — hero section becomes visible underneath
//     const timer2 = setTimeout(() => {
//       if (overlayRef.current) {
//         overlayRef.current.style.transition = "opacity 0.6s ease-out";
//         overlayRef.current.style.opacity = "0";
...
//     // Step 4: Mark complete — unmount overlay + logo
//     const timer4 = setTimeout(() => {
//       setAnimationComplete(true);
//       // TEMPORARILY DISABLED - Not saving to session for testing
//       // sessionStorage.setItem('uxbyte_intro_seen', 'true');
...
//       {shouldPlayAnimation && (
//         <>
//           {/* Overlay blocks the hero until the animation completes */}
//           <div
//             ref={overlayRef}
//             className="fixed inset-0 bg-white z-50 pointer-events-none"
//             style={{ opacity: 1 }}
...
//           {/* Logo: starts centered, slides to top-left, then fades out */}
//           {!animationComplete && (
//             <div
//               ref={logoRef}
//               className="fixed z-[60] pl-2 md:pl-0 flex items-center gap-1 md:gap-2"
//               style={{
//                 left: "50%",
//                 top: "50%",
//                 transform: "translate(-50%, -50%) scale(2)",
...
import { useEffect, useLayoutEffect, useRef, useState } from "react";
import { ChevronDown, Menu, X, ArrowRight, Link } from "lucide-react";
import { motion } from "framer-motion";
import MobileCenterCarousel from "./card-caruosel";
import Title from "@/commonComponents/title";
...
export default function HeroNew() {
  ...
  useEffect(() => {
    ...
    // Step 1: After 600ms, animate logo from center → top-left
    const timer1 = setTimeout(() => {
      if (!logoRef.current) return;
      ...
      if (isMobile) {
        logoRef.current.style.left = "80px";
        logoRef.current.style.top = "30px";
        logoRef.current.style.transform = "translate(0, 0) scale(1)";
      } else {
        logoRef.current.style.left = "100px";
        logoRef.current.style.top = "24px";
    ...
    // Step 2: Fade out overlay — hero becomes visible underneath
    const timer2 = setTimeout(() => {
      if (overlayRef.current) {
        overlayRef.current.style.transition = "opacity 0.6s ease-out";
        overlayRef.current.style.opacity = "0";
    ...
    // Step 3: Fade out the intro logo
    const timer3 = setTimeout(() => {
      if (logoRef.current) {
        logoRef.current.style.transition = "opacity 0.5s ease-out";
        logoRef.current.style.opacity = "0";
    ...
    // Step 4: Unmount intro logo AFTER it has fully faded (3400ms)
    // This must be AFTER timer3's fade (2900 + 500ms fade duration)
    const timer4 = setTimeout(() => {
      setAnimationComplete(true);
      // TEMPORARILY DISABLED - Not saving to session for testing
      // sessionStorage.setItem('uxbyte_intro_seen', 'true');
      ...
      <section>
        ...
        <div className="flex flex-col md:hidden overflow-x-hidden min-h-screen">
          ...
          <div className="w-full flex justify-center mt-10">
            <Button variant="outlined" label="Contact Us" />
          </div>
        </div>
      </section>
    </>
